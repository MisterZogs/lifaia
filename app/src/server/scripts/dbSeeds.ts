...
function generateMockUserData(): MockUserData {
  ...
  return {
    ...
    subscriptionPlan: subscriptionStatus
      ...
      : null,
    // Champs profil patient Lifaia
    pays: faker.helpers.arrayElement(['FR', 'BE', 'CH', 'US', 'GB']),
    langue: faker.helpers.arrayElement(['fr-FR', 'en-US', 'en-GB']),
    cgAccepteeAt: faker.date.past({ refDate: now }),
    disclaimerMedicalAccepteAt: faker.date.past({ refDate: now }),
    ...
    tailleCm: faker.number.int({ min: 155, max: 195 }),
    poidsKg: faker.number.float({ min: 50, max: 100, precision: 0.1 }),
    groupeSanguin: faker.helpers.arrayElement(['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-', null]),
