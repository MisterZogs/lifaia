...
npm ci
VITE_REACT_APP_API_URL=https://api.lifaia.com npm run build
cd -

echo "=== 3. Build image Docker du serveur ==="
docker build -t mydoctoria-server:latest .wasp/build/server/
...
rsync -avz --delete "$APP_DIR/.wasp/build/web-app/dist/" "$VPS_HOST:/var/www/lifaia/"
